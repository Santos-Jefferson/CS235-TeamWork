...
// forward declaration for QueueConstIterator
template <class T>
...
class QueueIterator
{
  ...
  QueueIterator(T * p) : p(p) {}
   ...
   // copy constructor
   QueueIterator(const QueueIterator & rhs) { *this = rhs; }

   // assignment operator
   QueueIterator & operator = (const QueueIterator & lhs)
   {
      this->p = lhs.p;
      return *this;
   }
   ...
   // not equals operator
   bool operator != (const QueueIterator & lhs) const
   {
      return lhs.p != this->p;
   }
